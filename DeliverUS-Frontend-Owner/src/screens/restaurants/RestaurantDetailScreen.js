...
import { getDetail, getRestaurantOrders } from '../../api/RestaurantEndpoints'
...
export default function RestaurantDetailScreen({ navigation, route }) {
  ...
  const [numPendingOrders, setNumPendingOrders] = useState(null)
  ...
  const renderHeader = () => {
    return (
      <View>
        ...
        <Pressable
          onPress={() => navigation.navigate('OrdersScreen', { id: restaurant.id })
          }
          style={({ pressed }) => [
            {
              backgroundColor: pressed
          ...
          <View style={[{ flex: 1, flexDirection: 'row', justifyContent: 'center' }]}>
            <MaterialCommunityIcons name='order-bool-ascending-variant' color={'white'} size={20} />
            <TextRegular textStyle={styles.text}>
              Check orders
            </TextRegular>
            {numPendingOrders !== null &&
              <TextRegular textStyle={styles.badge}>
                {numPendingOrders}
              </TextRegular>
            }
          </View>
  ...
  const getNumPendingOrders = async () => {
    try {
      const pendingOrders = await getRestaurantOrders(route.params.id, { status: 'pending' })
      setNumPendingOrders(pendingOrders.length)
    } catch (error) {
      showMessage({
        message: `There was an error while retrieving pending orders for restaurant (id ${route.params.id}). ${error}`,
        type: 'error',
        style: GlobalStyles.flashStyle,
        titleStyle: GlobalStyles.flashTextStyle
      })
